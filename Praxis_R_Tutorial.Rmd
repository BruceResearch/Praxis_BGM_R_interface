...
topf_py <- bf_results$top_features

# Force to numpy ndarray on Python side
topf_np <- np$asarray(topf_py)

# Convert to R
topf_r <- py_to_r(topf_np)

# Make sure it's a plain integer vector
topf <- as.integer(as.vector(topf_r))
